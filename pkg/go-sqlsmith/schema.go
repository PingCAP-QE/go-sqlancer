...
func (s *SQLSmith) LoadSchema(records [][6]string, indexes map[string][]string) {
	// init databases
	for _, record := range records {
		...
		if _, ok := s.Databases[dbname].Tables[tableName].Columns[columnName]; !ok {
			dataLenReg := regexp.MustCompile(typeRegex)
			dataLengthStr := dataLenReg.FindString(columnType)
			length := 0
			if dataLengthStr != "" {
			...
			s.Databases[dbname].Tables[tableName].Columns[columnName] = &types.Column{
				...
				DataType: dataLenReg.ReplaceAllString(strings.ToLower(columnType), ""),
				DataLen:  length,
				Options:  options,
			}
		}
