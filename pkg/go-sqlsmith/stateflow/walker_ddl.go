...
import (
	...
	"github.com/chaos-mesh/private-wreck-it/pkg/go-sqlsmith/data_gen"
...
func (s *StateFlow) walkCreateIndexStmt(node *ast.CreateIndexStmt) (*types.Table, error) {
	...
	for _, column := range table.Columns {
		...
		if column.DataType == "text" {
			length := s.rand.Intn(31) + 1
			name = fmt.Sprintf("%s(%d)", name, length)
		} else if column.DataType == "varchar" {
			length := 1
			if column.DataLen > 1 {
				maxLen := mathutil.Min(column.DataLen, 32)
				length = s.rand.Intn(maxLen-1) + 1
			}
			name = fmt.Sprintf("%s(%d)", name, length)
		}
	...
	if len(node.IndexPartSpecifications) > 10 {
		node.IndexPartSpecifications = node.IndexPartSpecifications[:s.rand.Intn(10)+1]
	}
...
func (s *StateFlow) walkPartitionDefinitionsTimestamp(definitions *[]*ast.PartitionDefinition) {
	for i := 0; i < len(timestampPartition); i += util.RdRange(1, 3) {
		val := driver.ValueExpr{}
		val.SetMysqlTime(tidbTypes.NewTime(tidbTypes.FromGoTime(data_gen.GenerateTimestampItem()), 0, 0))
		*definitions = append(*definitions, &ast.PartitionDefinition{
			Name: model.NewCIStr(fmt.Sprintf("p%d", i)),
			Clause: &ast.PartitionDefinitionClauseLessThan{
				Exprs: []ast.ExprNode{
