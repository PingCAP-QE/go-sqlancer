...
func (p *SQLancer) progress() {
	...
	case approachNoREC, approachTLP:
		...
		if approach == approachTLP {
			supportTps := []uint8{transformer.ON_CONDITION, transformer.WHERE}
			transformers = []transformer.Transformer{
				&transformer.TLPTrans{
					Expr: &ast.ParenthesesExpr{Expr: p.ConditionClause(genCtx, 2)},
					// because we donnot support generate HAVING and GROUP BY clause
					// so disable HAVING Tp
					Tp: supportTps[Rd(len(supportTps))],
					// Tp:   transformer.RandTLPType(),
				}}
		}
		_ = p.withTxn(RdBool(), func() error {
			// fix TLP applied on a sql generated by TLP, it will cause error 1250
			// TODO: give special table name to subqueries
			nodesGroup := transformer.NewTransformer(transformers, 1).Transform([][]ast.ResultSetNode{{selectAst}})
